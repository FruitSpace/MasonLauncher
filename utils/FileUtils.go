...
func DownloadCustomTextures(srvId string, textures string) error {
	log.Println(srvId, textures)
	pwd := CreateWorkdir()
	if FileExists(pwd + "/" + srvId + "_textures.zip") {
		DownloadingNow <- 1
		return nil
	}
	return DownloadFile("https://cdn.fruitspace.one/gdps_textures/"+textures, pwd+"/"+srvId+"_textures.zip")
}
